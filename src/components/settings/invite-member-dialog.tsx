...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our WhatsApp CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our WhatsApp CRM account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our WhatsApp CRM account';
    const message = `Join ${accountName} on WhatsApp CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
